...
# 단계 2: 요인분석 - 요인회전법 적용(Varimax 회전법이 기본)
# 단계 2-2: 고유값으로 가정한 3개 요인으로 분석, 배리맥스 회전법 사용, regression으로 요인점수 계산
...
print(result$loadings, cutoff = 0) # 모든 요인적재량 보기(숨겨진 요인적재량까지)
...
# 요인분석
result3 <- factanal(dw_df, factors = 3, rotation = "varimax")
result3
